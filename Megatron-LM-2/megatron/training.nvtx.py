...
def train_step_profile(forward_step_func, data_iterator,
               model, optimizer, opt_param_scheduler):
    ...
    layer_time[-1] += module_time[-1]
    print_rank_0(f"layer_time:\n{layer_time}\n\n\n")
    layer_time=[]
    # Empty unused memory.
...
def train_step(forward_step_func, data_iterator,
    ...
    timers = get_timers()
    # add for nvtx
    batch = args.curr_iteration
    tp_rank = mpu.get_tensor_model_parallel_rank()
    pp_rank = mpu.get_pipeline_model_parallel_rank()
    dp_rank = mpu.get_data_parallel_rank()
    
    with nvtx.annotate(f"rank {tp_rank, dp_rank, pp_rank} step {batch}",color="blue"):
        torch.cuda.synchronize()        
        with nvtx.annotate(f"rank {tp_rank, dp_rank, pp_rank} zero_grad {batch}", color="yellow"):            
            # Set grad to zero.
            if args.DDP_impl == 'local' and args.use_contiguous_buffers_in_local_ddp:
                for partition in model:
                    partition.zero_grad_buffer()
        ...
        with nvtx.annotate(f"rank {tp_rank, dp_rank, pp_rank} fpbp {batch}", color="purple"):
            torch.cuda.synchronize()                    
            # Forward pass.
        ...
        with nvtx.annotate(f"rank {tp_rank, dp_rank, pp_rank} reduce_gradients {batch}", color="pink"):
            torch.cuda.synchronize()            
            # Reduce gradients.
            optimizer.reduce_model_grads(args, timers)
        ...
        with nvtx.annotate(f"rank {tp_rank, dp_rank, pp_rank} update {batch}", color="green"):
            torch.cuda.synchronize()            
            # Update parameters.
            timers('optimizer', log_level=1).start(barrier=args.barrier_with_L1_time)
            update_successful, grad_norm, num_zeros_in_grad = optimizer.step(args, timers)
        ...
        with nvtx.annotate(f"rank {tp_rank, dp_rank, pp_rank} average loss {batch}", color="orange"):
            torch.cuda.synchronize()            
            if mpu.is_pipeline_last_stage(ignore_virtual=True):
                # Average loss across microbatches.
                loss_reduced = {}
